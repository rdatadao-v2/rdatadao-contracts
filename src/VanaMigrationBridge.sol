...
contract VanaMigrationBridge is IMigrationBridge, AccessControl, Pausable, ReentrancyGuard {
     ...
     */
    constructor(address _v2Token, address _admin, address[] memory validators_) {
     ...
     */
    function submitValidation(address user, uint256 amount, bytes32 burnTxHash, uint256 burnBlockNumber)
        external
        override
        nonReentrant
        whenNotPaused
        onlyRole(VALIDATOR_ROLE)
    {
        ...
        if (
            request.validatorApprovals >= MIN_VALIDATORS && block.timestamp >= request.challengeEndTime
                && !request.challenged
        ) {
     ...
     */
    function challengeMigration(bytes32 requestId) external override onlyRole(VALIDATOR_ROLE) whenNotPaused {
        MigrationRequest storage request = _migrationRequests[requestId];
