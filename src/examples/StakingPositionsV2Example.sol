...
contract StakingPositionsV2Example is StakingPositions {
     ...
     */
    function stakeWithReferral(uint256 amount, uint256 lockPeriod, address referrer)
        external
        whenNotPaused
     ...
     */
    function calculateLoyaltyPoints(uint256 amount, uint256 lockPeriod) public pure returns (uint256) {
     ...
     */
    function calculatePositionBoost(address user, uint256, uint256) public view returns (uint256) {
    ...
    function getPositionWithBoost(uint256 positionId)
        external
        view
        returns (IStakingPositions.Position memory position, uint256 boost, uint256 projectedRewards)
    {
