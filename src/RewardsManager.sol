    ...
    IRewardsManager
{
     ...
     */
    function registerProgram(address rewardModule, string calldata name, uint256 startTime, uint256 duration)
        external
        override
     ...
     */
    function emergencyPauseProgram(uint256 programId) external override onlyRole(PAUSER_ROLE) validProgram(programId) {
     ...
     */
    function notifyStake(address user, uint256 stakeId, uint256 amount, uint256 lockPeriod)
        external
        override
        onlyStakingManager
        whenNotPaused
     ...
     */
    function notifyUnstake(address user, uint256 stakeId, bool emergency) external override onlyStakingManager {
     ...
     */
    function claimRewards(uint256 stakeId) external override whenNotPaused nonReentrant returns (ClaimInfo[] memory) {
     ...
     */
    function claimAllRewards() external override whenNotPaused nonReentrant returns (ClaimInfo[] memory) {
        ...
        for (uint256 i = 0; i < programIds.length; i++) {
            ...
            if (_isProgramActive(program)) {
                try IRewardModule(program.rewardModule).claimRewards(user, stakeId) returns (uint256 amount) {
                    if (amount > 0) {
                        claims[claimIndex++] =
                            ClaimInfo({programId: programId, token: program.rewardToken, amount: amount});
                    }
                } catch {
    ...
    function notifyRevenueReward(uint256 amount) external override {
        ...
        for (uint256 i = 0; i < programIds.length; i++) {
            ...
            if (_isProgramActive(program) && keccak256(bytes(program.name)) == keccak256(bytes("RDAT Staking Rewards")))
            {
                // Use low-level call since notifyRewardAmount is not in base interface
                (bool success,) =
                    program.rewardModule.call(abi.encodeWithSignature("notifyRewardAmount(uint256)", amount));
                if (success) {
    ...
    function _isProgramActive(RewardProgram memory program) private view returns (bool) {
        return program.active && !program.emergency && block.timestamp >= program.startTime
