...
contract TimelockIntegration {
     ...
     */
    function setupTimelockForRDAT(address rdatToken, address timelock, address currentAdmin) external {
     ...
     */
    function scheduleUpgrade(address timelock, address implementation, address rdatProxy)
        external
        returns (bytes32 operationId)
    {
        ...
        bytes memory upgradeCall = abi.encodeWithSignature("upgradeToAndCall(address,bytes)", implementation, "");

        // Calculate operation ID
        operationId =
            keccak256(abi.encode(rdatProxy, 0, upgradeCall, bytes32(0), keccak256(abi.encode(block.timestamp))));
     ...
     */
    function executeUpgrade(address timelock, address implementation, address rdatProxy, bytes32 salt) external {
        TimelockController controller = TimelockController(payable(timelock));
     ...
     */
    function emergencyCancel(address timelock, bytes32 operationId) external {
        TimelockController controller = TimelockController(payable(timelock));
     ...
     */
    function isOperationReady(address timelock, bytes32 operationId) external view returns (bool) {
        TimelockController controller = TimelockController(payable(timelock));
     ...
     */
    function getOperationTimestamp(address timelock, bytes32 operationId) external view returns (uint256) {
        TimelockController controller = TimelockController(payable(timelock));
