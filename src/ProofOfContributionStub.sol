...
contract ProofOfContributionStub is IProofOfContribution, IProofOfContributionIntegration, AccessControl {
    ...
    constructor(address _admin, address _dlpAddress) {
        ...
        systemState = SystemState({dlpAddress: _dlpAddress, currentEpoch: 1, isActive: true});
    ...
    function validateContribution(address contributor, uint256 contributionId) external override {
    ...
    function contributions(address, uint256) external view override returns (Contribution memory) {
        return Contribution({timestamp: block.timestamp, score: 0, dataHash: bytes32(0), validated: false});
    }
