...
contract MockRewardsManager is IRewardsManager {
    ...
    function notifyStake(address user, uint256 stakeId, uint256 amount, uint256 lockPeriod) external override {
        // Mock implementation - do nothing
    }

    function notifyUnstake(address user, uint256 stakeId, bool emergency) external override {
        // Mock implementation - do nothing
    }

    // Required but not used in tests
    function registerProgram(address, string calldata, uint256, uint256) external pure override returns (uint256) {
        return 0;
    }
    ...
    function calculateRewards(address, uint256)
        external
        pure
        override
    ...
    function calculateAllRewards(address)
        external
        pure
        override
        returns (uint256[] memory amounts, address[] memory tokens)
