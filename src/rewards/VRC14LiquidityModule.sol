...
contract VRC14LiquidityModule is IRewardModule, AccessControl, ReentrancyGuard {
    ...
    // ========== CONSTRUCTOR ==========
    constructor(address _rdatToken, address _stakingManager, address _admin) {
     ...
     */
    function configureUniswap(address _vanaToken, address _swapRouter, address _positionManager, address _factory)
        external
        onlyRole(ADMIN_ROLE)
    {
    ...
    function createOrSetPool(uint24 _poolFee) external onlyRole(ADMIN_ROLE) {
        ...
        address existingPool = uniswapFactory.getPool(address(rdatToken), address(vanaToken), poolFee);
     ...
     */
    function claimRewards(address user, uint256)
        external
        override
     ...
     */
    function _addLiquidity(uint256 rdatAmount, uint256 vanaAmount)
        private
        returns (uint256 tokenId, uint128 liquidity)
    {
        ...
        (uint256 amount0, uint256 amount1) =
            token0 == address(rdatToken) ? (rdatAmount, vanaAmount) : (vanaAmount, rdatAmount);
    ...
    function onUnstake(address user, uint256 stakeId, uint256 amount, bool emergency)
        external
        override
        onlyRole(REWARDS_MANAGER_ROLE)
    {
