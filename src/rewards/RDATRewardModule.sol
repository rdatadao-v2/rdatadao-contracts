...
contract RDATRewardModule is IRewardModule, AccessControl, ReentrancyGuard {
    ...
    function claimRewards(
        address user,
        uint256 stakeId
    ) external override onlyRewardsManager nonReentrant returns (uint256) {
        _updateRewards(user, stakeId);
    ...
    function remainingAllocation() external view override returns (uint256) {
        return totalAllocation > totalDistributedAmount ? 
            totalAllocation - totalDistributedAmount : 0;
    }
    
    function getRewardState(address user, uint256 stakeId) 
        external 
        view 
    ...
    function emergencyWithdraw(address token, uint256 amount) 
        ...
        onlyRole(ADMIN_ROLE) 
    {
        uint256 remaining = totalAllocation > totalDistributedAmount ? 
            totalAllocation - totalDistributedAmount : 0;
