...
contract vRDATRewardModule is IRewardModule, AccessControl, ReentrancyGuard {
    ...
    constructor(address _vrdatToken, address _stakingManager, address _rewardsManager, address _admin) {
        if (
            _vrdatToken == address(0) || _stakingManager == address(0) || _rewardsManager == address(0)
                || _admin == address(0)
        ) {
     ...
     */
    function onUnstake(address user, uint256 stakeId, uint256, bool emergency)
        external
        override
        onlyRewardsManager
        nonReentrant
     ...
     */
    function calculateRewards(address, uint256) external pure override returns (uint256) {
     ...
     */
    function claimRewards(address, uint256) external view override onlyRewardsManager returns (uint256) {
