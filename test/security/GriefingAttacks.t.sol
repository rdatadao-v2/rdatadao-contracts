...
contract GriefingAttacksTest is Test {
    ...
    function test_ReentrancyProtectionDuringStake() public {
        ...
        MaliciousReentrant malicious = new MaliciousReentrant(address(stakingPositions), address(rdat));
    ...
    function test_EmergencyWithdrawFailsWithoutVRDAT() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                bytes4(keccak256("AccessControlUnauthorizedAccount(address,bytes32)")), victim, vrdat.BURNER_ROLE()
            )
        );
