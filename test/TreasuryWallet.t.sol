...
contract TreasuryWalletTest is Test {
    ...
    function setUp() public {
        ...
        bytes memory treasuryBytecode =
            abi.encodePacked(type(ERC1967Proxy).creationCode, abi.encode(address(implementation), ""));
        address predictedTreasury = address(
            uint160(
                uint256(keccak256(abi.encodePacked(bytes1(0xff), address(this), salt, keccak256(treasuryBytecode))))
            )
        );

        // Deploy RDAT proxy with predicted treasury address
        bytes memory rdatInitData =
            abi.encodeCall(RDATUpgradeable.initialize, (predictedTreasury, admin, migrationBridge));
        ...
        bytes memory treasuryInitData = abi.encodeCall(TreasuryWallet.initialize, (admin, address(rdat)));
