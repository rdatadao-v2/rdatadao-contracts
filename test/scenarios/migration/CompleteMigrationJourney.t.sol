...
contract CompleteMigrationJourney is Test {
    ...
    function _setupVanaChain() private {
        ...
        bytes memory stakingInitData =
            abi.encodeCall(stakingImpl.initialize, (address(v2Token), address(vrdatToken), admin));
        ...
        vanaBridge = new VanaMigrationBridge(address(v2Token), admin, validators);
    ...
    function test_BonusDecay_WeekByWeek() public {
        ...
        for (uint256 i = 0; i < tests.length; i++) {
            ...
            assertEq(bonus, expectedBonus, string.concat("Week ", vm.toString(tests[i].week), " bonus incorrect"));

            console2.log(
                string.concat(
                    "Week ", vm.toString(tests[i].week), " bonus: ", vm.toString(tests[i].expectedPercent), "%"
                )
            );
        }
    ...
    function test_MultiUser_ConcurrentMigrations() public {
        ...
        console2.log(
            string.concat(
                "   - Daily limit used: ",
                vm.toString((vanaBridge.dailyMigrated() * 100) / vanaBridge.DAILY_LIMIT()),
                "%"
            )
        );
