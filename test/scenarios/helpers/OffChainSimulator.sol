...
contract OffChainSimulator is Test {
     ...
     */
    function simulateValidatorNetwork(address user, uint256 amount, bytes32 burnTxHash, uint256 baseBlockNumber)
        external
    ...
    function addValidator(address validator) external {
        ...
        validators[validator] =
            ValidatorState({isActive: true, isOnline: true, reputation: 100, lastSeen: block.timestamp});
    ...
    function canExecuteMigration(bytes32 requestId) external view returns (bool) {
        MigrationRequest storage request = migrationRequests[requestId];
        return request.validationsReceived >= MIN_VALIDATORS_FOR_CONSENSUS && !request.isChallenged
            && block.timestamp >= request.challengeDeadline && !request.isProcessed;
    }

    /**
     * @notice Gets snapshot proposal results
     */
    function getSnapshotResults(bytes32 proposalId)
        external
        view
        returns (uint256 forVotes, uint256 againstVotes, uint256 abstainVotes, bool passed, bool finalized)
    {
        SnapshotProposal storage proposal = snapshotProposals[proposalId];
        return (proposal.forVotes, proposal.againstVotes, proposal.abstainVotes, proposal.passed, proposal.isFinalized);
    }
