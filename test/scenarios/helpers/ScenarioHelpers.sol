...
contract ScenarioHelpers is Test {
    ...
    function fundUser(address user, uint256 v1Amount, uint256 ethAmount) external {
        ...
        console2.log(
            string.concat(
                "[FUND] Funded ", userProfiles[user].name, " with ", vm.toString(v1Amount / 1e18), " V1 tokens"
            )
        );
    }
    ...
    function setupUserStaking(address user, uint256[] memory amounts, uint256[] memory periods) external {
        ...
        for (uint256 i = 0; i < amounts.length; i++) {
            ...
            console2.log(
                string.concat(
                    "[CHART] Staked ",
                    vm.toString(amounts[i] / 1e18),
                    " RDAT for ",
                    vm.toString(periods[i] / 1 days),
                    " days"
                )
            );
        }
        ...
        console2.log(
            string.concat(
                "[OK] Setup ", vm.toString(amounts.length), " staking positions for ", userProfiles[user].name
            )
        );
    }
    ...
    function snapshotSystemState() external returns (bytes32 snapshotId) {
        ...
        snapshot.stateHash = keccak256(
            abi.encodePacked(
                snapshot.totalRDATSupply, snapshot.totalvRDATSupply, snapshot.totalStaked, snapshot.activePositions
            )
        );
    ...
    function expectGovernanceEvent(uint256 proposalId, uint8 outcome) external pure {
        ...
        console2.log(
            string.concat(
                "[SEARCH] Expecting governance event for proposal ",
