...
contract DataContributionJourney is Test {
    ...
    function _setupCore() private {
        ...
        bytes memory stakingInitData = abi.encodeCall(
            ...
            (address(rdatToken), address(vrdatToken), admin)
        );
    ...
    function _setupRewardsSystem() private {
        ...
        rewardModule = new vRDATRewardModule(
            address(vrdatToken),
            address(staking),
            address(rewardsManager),
            admin
    ...
    function test_CompleteDataContributionFlow() public {
        ...
        rdatToken.createDataPool(
            poolId,
            submission.ipfsHash,
            initialContributors
        );
        
        bool success = rdatToken.addDataToPool(
            poolId,
            submission.dataHash,
    ...
    function test_MultiContributor_DifferentKismetTiers() public {
        ...
        for (uint256 i = 0; i < contributors.length; i++) {
            ...
            console2.log(string.concat("   ", tiers[i], " contributor submitted (quality: ", vm.toString(baseQuality), ")"));
        }
    ...
    function test_DataQualityScoring() public {
        ...
        for (uint256 i = 0; i < submissions.length; i++) {
            ...
            uint256 qualityScore = _calculateDataQuality(
                sub.posts,
                sub.comments,
                sub.karma
            );
     ...
     */
    function _calculateDataQuality(
        uint256 posts,
        uint256 comments,
        uint256 karma
        ...
        for (uint256 i = 0; i < 4; i++) {
            ...
            console2.log(
                string.concat(
                    ...
                    tiers[i],
                    " (",
                    vm.toString(multipliers[i]),
                    "%): ",
                    vm.toString(reward / 1e18),
                    " RDAT"
                )
            );
        }
