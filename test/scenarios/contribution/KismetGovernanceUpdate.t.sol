...
contract KismetGovernanceUpdate is Test {
    ...
    function test_ProposalToUpdateKismetFormula() public {
        ...
        (uint256 forVotes, uint256 againstVotes, uint256 abstainVotes,,) = simulator.getSnapshotResults(snapshotId);
        ...
        console2.log(string.concat("   - Abstain: ", vm.toString(abstainVotes / 1e18), " vRDAT"));
    ...
    function _displayFormula(KismetTier[] memory formula) internal pure {
        for (uint256 i = 0; i < formula.length; i++) {
            ...
            if (formula[i].dataQualityBonus > 0) {
                console2.log(string.concat("     + Quality bonus: ", vm.toString(formula[i].dataQualityBonus), "%"));
            }
        }
    }
}
...
contract KismetCalculator {
    ...
    function scheduleFormulaUpdate(KismetGovernanceUpdate.KismetTier[] memory newFormula)
        external
    ...
    function emergencyUpdateFormula(KismetGovernanceUpdate.KismetTier[] memory emergencyFormula) external {
        require(msg.sender == admin, "Only admin");
    ...
    function calculateKismet(uint256 reputation, uint256 dataQuality) external view returns (uint256 multiplier) {
        require(!paused, "Calculator paused");

        // Find appropriate tier
        for (uint256 i = 0; i < activeTiers.length; i++) {
            if (reputation >= activeTiers[i].minReputation && reputation <= activeTiers[i].maxReputation) {
