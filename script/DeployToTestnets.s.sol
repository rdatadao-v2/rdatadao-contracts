...
contract DeployToTestnets is Script {
    ...
    function getChainConfig() internal view returns (address admin, address treasuryAddr, string memory chainName) {
        ...
        } else if (chainId == 1480) {
            // Vana Mainnet (for future)
            admin = 0x29CeA936835D189BD5BEBA80Fe091f1Da29aA319;
            treasuryAddr = 0x29CeA936835D189BD5BEBA80Fe091f1Da29aA319;
    ...
    function deployCore(address admin, address treasuryAddr) internal {
        ...
        migrationBridge = new VanaMigrationBridge(predictedRdatAddress, admin, validators);
        ...
        bytes memory rdatInitData =
            abi.encodeWithSelector(RDATUpgradeable.initialize.selector, treasuryAddr, admin, address(migrationBridge));
        ...
        bytes memory stakingInitData = abi.encodeWithSelector(
            StakingPositions.initialize.selector, address(rdat), address(vrdat), admin, address(emergencyPause)
        );
    ...
    function deployFinancial(address admin, address treasuryAddr) internal {
        ...
        bytes memory treasuryInitData = abi.encodeWithSelector(TreasuryWallet.initialize.selector, admin, address(rdat));
