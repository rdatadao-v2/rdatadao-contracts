...
contract DeployWithCREATE2 is Script {
    ...
    function run() external returns (DeploymentAddresses memory addresses) {
        ...
        address migrationBridge =
            vm.envOr("MIGRATION_BRIDGE_ADDRESS", address(0x70997970C51812dc3A010C7d01b50e0d17dc79C8));
        ...
        addresses.staking =
            address(deployStaking(addresses.rdatProxy, addresses.vrdat, addresses.emergencyPause, admin));
    ...
    function deployFactory() internal returns (address) {
        ...
        address predictedFactory = computeCreate2Address(factoryBytecode, FACTORY_SALT, address(this));
    ...
    function calculateRDATAddress(address treasuryAddress, address admin, address migrationBridge)
        internal
        view
        returns (address)
    {
        ...
        bytes memory initData = abi.encodeCall(RDATUpgradeable.initialize, (treasuryAddress, admin, migrationBridge));
    ...
    function deployEmergencyPause(address admin) internal returns (EmergencyPause) {
        bytes memory bytecode = abi.encodePacked(type(EmergencyPause).creationCode, abi.encode(admin));
    ...
    function deployVRDAT(address rdatAddress, address admin) internal returns (vRDAT) {
        bytes memory bytecode = abi.encodePacked(type(vRDAT).creationCode, abi.encode(rdatAddress, admin));
        address deployed = factory.deploy(bytecode, VRDAT_SALT);
        return vRDAT(deployed);
    }

    function deployRDAT(address treasuryAddress, address admin, address migrationBridge) internal returns (address) {
    ...
    function deployStaking(address rdatToken, address vrdatToken, address emergencyPauseAddress, address admin)
        internal
        returns (StakingPositions)
    {
        bytes memory bytecode = abi.encodePacked(
            type(StakingPositions).creationCode, abi.encode(rdatToken, vrdatToken, emergencyPauseAddress, admin)
        );
    ...
    function computeCreate2Address(bytes memory bytecode, bytes32 salt, address deployer)
        internal
        pure
        returns (address)
    {
