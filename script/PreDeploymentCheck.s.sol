...
contract PreDeploymentCheck is Script {
    ...
    function checkDeployerKey() internal view returns (CheckResult memory) {
        ...
        if (deployer == EXPECTED_DEPLOYER) {
            return CheckResult(
                true, string.concat(unicode"[OK] Deployer address matches expected: ", vm.toString(deployer))
            );
        } else {
            return CheckResult(
                false,
    ...
    function checkDeployerBalance() internal view returns (CheckResult memory) {
        ...
        if (balance >= MIN_DEPLOYMENT_BALANCE) {
            return CheckResult(true, string.concat(unicode"[OK] Sufficient balance: ", formatEther(balance), " ETH"));
        } else {
            return CheckResult(
                false,
                string.concat(
    ...
    function checkMultisigSetup(uint256 chainId) internal view returns (CheckResult memory) {
        ...
        if (hasCode || hasBalance) {
            ...
            return CheckResult(
                false, string.concat(unicode"⚠ Multisig may not be deployed at: ", vm.toString(expectedMultisig))
            );
        }
    }
    ...
    function checkNetworkConnectivity() internal view returns (CheckResult memory) {
        ...
        try this.getBlockNumber() returns (uint256 blockNum) {
            return CheckResult(
                true, string.concat(unicode"[OK] Connected to network, block height: ", vm.toString(blockNum))
            );
        } catch {
            return CheckResult(false, unicode"✗ Network connectivity issue");
        }
    }
    ...
    function checkGasPrice() internal view returns (CheckResult memory) {
        ...
        if (gasPrice <= maxAcceptableGasPrice) {
            return CheckResult(
                true, string.concat(unicode"[OK] Gas price acceptable: ", vm.toString(gasPrice / 1 gwei), " gwei")
            );
        } else {
            return CheckResult(
                false,
                string.concat(unicode"⚠ High gas price: ", vm.toString(gasPrice / 1 gwei), " gwei (consider waiting)")
            );
        }
    }
    ...
    function checkContractSizes() internal pure returns (CheckResult memory) {
        ...
        bool allWithinLimit = rdatSize < maxSize && vrdatSize < maxSize && stakingSize < maxSize && bridgeSize < maxSize;

        if (allWithinLimit) {
            return CheckResult(true, unicode"[OK] All contracts within size limits");
        } else {
            return CheckResult(false, unicode"✗ Some contracts may exceed size limit");
        }
    }
    ...
    function formatEther(uint256 weiAmount) internal pure returns (string memory) {
        ...
        return string.concat(vm.toString(ethPart), ".", padZeros(vm.toString(decimalPart), 3));
    }
